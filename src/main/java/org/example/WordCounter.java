...
public class WordCounter {
    ...
    public static Map<String,Integer> calculatedWord(){
        ...
        String[] temizlenmis = text.toLowerCase().replaceAll("[^a-zA-Z\\s]","").split("\\s+");

        List<String> list = Arrays.asList(temizlenmis);
        Map<String,Integer> donusMap = new HashMap<>();
        for(String word: list){
            donusMap.put(word,Collections.frequency(list,word));
        }
